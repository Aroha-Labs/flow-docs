...
After setting up the integration, you'll need two key pieces of information:
...
response = client.flow.test(
    ...
    input_dict,
    ComposioConfig(
        COMPOSIO_API_KEY="YOUR_COMPOSIO_API_KEY",  # From step 2
        ACTION="PLATFORM_ACTION",  # e.g., TWITTER_CREATION_OF_A_POST
        TASK="Action specific task - {content}",
        ENTITY_ID="YOUR_ENTITY_ID"  # From step 2
    )
)
```