...
For leveraging different input types such as file and URL, follow the below code snippet:
...
flow = Flow(source = "FLOW.yml")

input_dict = {"input1" : File(file_path="user.txt"), "input2" : Reader(url="https://john-doe.com/"), "input3" : "John Doe"}

response = client.flow.test(flow, input_dict)
print(response)
```

The official YAML structure for a flow with file and URL input capabilities:
...
prompt: |
  Your flow's primary instruction or role...
  You can use {input1} , {input2}, {input3} placeholders to reference inputs.
